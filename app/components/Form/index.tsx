...
export default function Form({config, showLogo = true, showProgress = true}: FormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Get the edit token from the response
      const registrationResult = await response.json()
      const registrationEditToken = registrationResult.editToken

      // Store edit token in state
      setEditToken(registrationEditToken)
      ...
      try {
        ...
        const emailResponse = await fetch('/api/send-registration-email', {
          ...
          body: JSON.stringify({...formData, editToken: registrationEditToken}),
        })
